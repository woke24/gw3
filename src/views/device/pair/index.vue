  ...
  <div class="choose-device-container">
    <div v-if="deviceDetected" class="choose-device-main-panel">
      <div class="choose-device-tip">
        <h2>{{ $t('message.device.chooseDevice') }}</h2>
      </div>
...
// 响应式获取 Vuex 实时数据
const allSittingDevices = computed(() => store.getters[`${NAMESPACE}/getSittingDevices`])
const allArmbandDevices = computed(() => store.getters[`${NAMESPACE}/getArmbandDevices`])
...
const chooseDevice = async (device) => {
  if (device) {
    ...
    switch (choosedDevice.deviceType) {
      case 2:
        router.push({ name: 'locosPair' })
        break
      case 3:
        router.push({ name: 'walkcPair' })
        break
      case 4:
        router.push({ name: 'walkc2Pair' })
        break
      case 8:
        router.push({ name: 'walkc2CorePair' })
        break
      case 9:
        router.push({ name: 'walkc2EnhancedPair' })
        break
    }
  }
}
